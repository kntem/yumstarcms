    ...
    $("#restaurantTable_delete").click(function() {
        ...
        if (restaurantTable_ids != '') {
            ...
            $.ajax({
                ...
                beforeSend: function() { /*jQuery("#indicator").show()8;*/ },
...
<div style="margin-bottom: 20px;">
    ...
    <div class="row">
        ...
        <div class="col-xs-5 col-md-6 text-right">
            <?php
                echo $this->Html->link('Genrate pdf', '/restaurantTables/viewpdf1/'.$restid, array('class' => 'btn btn-warning'));
                echo $this->Html->link('Add New', '/restaurantTables/add/'.$restid, array('class' => 'btn btn-primary'));
                echo $this->Form->button('Delete Selected', array('class' => 'btn btn-danger','id' => 'restaurantTable_delete'), __('are u delete?'));
            ?>
        </div>
    </div>
</div>
...
//        echo $this->QrCode->url($ar1); ?>
    ...
    <div class="pagination_outer pagination pagination-right pagination-mini" id="tablePagination">
        <span id="tablePagination_perPage">
            <?php
