<script type="text/javascript">
    ...
    //for item_choice_lidting
       var options = {
        ...
        $("#item_choice_list").tablesorter({headers: {0: {sorter: false}}});
...
//                    alert(jQuery('#SITE_URL').val());
                    ...
                    $.ajax({//fetch the article via ajax            
                        type: "POST",
                        url: 'items/choice_builk_delete', //calling this page
    ...
    function save_choice(id, data_item) {
        $.ajax({
            ...
            success: function(data) {
                ...
                $('#row_' + id + ' .lblname_' + id).each(function() {
                    $(this).replaceWith(obj.choice_name);
                });
                $('#row_' + id + ' .lbldesc_' + id).each(function() {
                    $(this).replaceWith(obj.description);
                });
                $('#row_' + id + ' .lblprice_' + id).each(function() {
                    $(this).replaceWith(obj.price);
                });
                $('.confirm_edit').text('Edit');
                $(".remove_inline_edit").remove();
...
<div class="widget widget-blue">
    ...
    <div class="widget-content">
        ...
        <div class="form-group">
            ...
            <div class="col-sm-8">
                <?php echo $this->Form->input('Item.rest_id', array('class' => 'required form-control', 'label' => false, 'type' => 'select', 'empty' => '','id'=>'restaurant_name', 'options' => $rest,'onchange'=>'selecRestaurant(this.options[this.selectedIndex].value)', 'selected' => $item['Item']['rest_id'])); ?>                
            </div>
        </div>
            <div class="form-group">
        ...
        <div class="form-group">
            ...
            <div class="col-sm-8">
                <?php echo $this->Form->input('Item.start_date', array('type' => 'text', 'id' => 'start_date', 'label' => false, 'div' => false, 'class' => 'required datepicker form-control', 'value' => $item['Item']['start_date'])); ?>
        ...
        <div class="form-group">
            ...
            <div class="widget widget-blue">
                ...
                <div class="widget-content">
                     <span class="input-group-btn" style="bottom:10px;">
                        
          <?php echo $this->Form->button('Delete Selected', array('class' => 'btn btn-danger','style' => 'float:right', 'id' => 'choice_delete'), __('are u delete?'));?>
                                ...
                                <?php foreach ($item_choice as $item_choices) { ?>
                                    <tr id="row_<?php echo $item_choices['ItemChoice']['id']; ?>">
                                        ...
                                        <td class="text-right">
                                            ...
                                            <?php echo $this->Form->postLink("<i class='icon-remove'></i>", array('action' => 'delete_choice', $item_choices['ItemChoice']['id']), array('class' => 'btn btn-danger btn-xs remove-tr', 'escape' => false, 'title' => 'Delete'), __('Are you sure you want to delete # %s?', $item_choices['ItemChoice']['choice_name'])); ?>
