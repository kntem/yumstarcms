...
//        $('#suggestion_list').tablePagination(options);
        $("#suggestion_list").tablesorter({headers: {0: {sorter: false},6: {sorter: false}}});
                    ...
                    $.ajax({//fetch the article via ajax            
                        ...
                        success: function(html) {
                            ...
                            if (response.is_deleted == 1) {
                               ...
                               if(rowCount == 1){                                
                              $('#suggestion_list > tbody:last').append('<tr><td colspan="8" class="fc-header-center">No Data found</td></tr>');
                               }
                            }
                        }
                    });
...
<div style="margin-bottom: 20px;">
    ...
    if (isset($flag)) {
        if ($flag == 'true') {
            ...
            echo $this->Html->link('View All', '/suggestions/report_abused', array('class' => 'btn btn-primary'));
        }
    }
    echo $this->Form->end();
    ?>    </div> 
    <div class="col-xs-5 col-md-6 text-right">
<?php echo $this->Form->button('Delete Selected', array('class' => 'btn btn-danger', 'id' => 'suggestion_delete'), __('are u delete?')); ?>
...
<div class="widget widget-blue">
    ...
    <div class="widget-content">
        ...
        <?php echo $this->Form->postLink("Confirm", array('action' => 'delete', $suggestion['Suggestion']['id']), array('class' => 'btn btn-default btn-xs', 'escape' => false, 'title' => 'Confirm'), __('Are you sure you want to delete this %s abused image?', $suggestion['Suggestion']['id'])); ?>
        <?php echo $this->Form->postLink("<i class='icon-remove'></i>", array('action' => 'change_abused_status', $suggestion['Suggestion']['id']), array('class' => 'btn btn-danger btn-xs ', 'escape' => false, 'title' => 'Delete'), __('Are you sure you want to declare this image not abused %S?', $suggestion['Suggestion']['id'])); ?>
                                    <!--<a class="btn btn-danger btn-xs remove-tr" href="#"><i class="icon-remove"></i></a>-->
                                </td>
                            </tr>
    <?php }
} else{
            ?> <tr><td colspan="8" class="fc-header-center">No Data Found</td></tr><?php
        }?>
                </tbody>
            </table>
