...
class QrCodeHelper extends AppHelper {
	...
	function text($text = '', $id) {
            $error_correction = 'M';
            $margin = 3;
           QRcode::png($text, WWW_ROOT . 'qrcode_images/qr_'.$id.'.png',$error_correction,$margin);
//		return $this->Html->image($this->base_url . urlencode($text) . $this->_optionsString($options),WWW_ROOT . 'category_images/gh.png' );
	}
	/**
	 ...
	 * @param mixed $options options array, see helper vars for description of parameters
	 */
	function id_qr($url = '',$id,$rid) {
//		$url = Router::url($url, TRUE);
//                $url=urlencode($url);
                 $error_correction = 'M';
            $margin = 3;
            
         return  QRcode::png($url, WWW_ROOT . 'qrcode_images/'.$rid.'/table_'.$id.'.png',$error_correction,$margin);
