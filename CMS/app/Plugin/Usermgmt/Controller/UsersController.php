...
class UsersController extends UserMgmtAppController {
     ...
     * @var array
     */
    public $uses = array('Usermgmt.User', 'Usermgmt.UserGroup', 'Usermgmt.LoginToken','Restaurant');
         public $components = array('Paginator');
        var $paginate = array('order'=>'User.id desc',
    ...
    public function index() {
        ...
        $conditions = array();
        if (($user_type != 'Admin')) {
            $conditions[] = array('OR' => array('User.rest_id =' => $rest_id));
        }
...
//                $conditions1[] = array('OR' => array('User.username LIKE' => '%' . $this->request->data['Search']['name'] . '%'));
        ...
        $user = $this->Paginator->paginate('User',$conditions);
        $this->set('users', $user);
...
//      $this->set('users', $users);
    ...
    public function login() {
        ...
        if ($this->request -> isPost()) {
            ...
            if($this->User->LoginValidate()) {
                ...
                $user1 = $this->User->findByEmail($email);

                $user = $this->User->findByUsername($email);
                if (empty($user)) {
                    $user = $this->User->findByEmail($email);
                    if (empty($user)) {
                        $this->Session->setFlash(__('Incorrect Email/Username or Password','default',array('class'=>'alert alert-danger alert-dismissable')));
                        return;
                    }
                }
                // check for inactive account
                if ($user['User']['id'] != 1 and $user['User']['active']==0) {
                    $this->Session->setFlash(__('Sorry your account is not active, please contact the Administrator'));
                    return;
                }
                // check for verified account
                if ($user['User']['id'] != 1 and $user['User']['email_verified']==0) {
                    $this->Session->setFlash(__('Your registration has not been confirmed, please verify your email or contact the Administrator'));
                    return;
                }
                ...
                if($user1['User']['user_group_id'] != $user_type) {
                    $this->Session->setFlash(__('You are not a Member of this group',array('class'=>'alert alert-danger alert-dismissable')));
                    return;
                }
    ...
    public function logout() {
        ...
        $this->Session->setFlash(__('You have successfully signed out'));
...
//        if($user_type != 'Admin'){
//            $this->request->data['User']['rest_id']=$rest_id;
//            $this->request->data['User']['user_group_id']=5;
//        }
        $userGroups=$this->UserGroup->getGroups();
        $this->set('userGroups', $userGroups);
        $restaurant=$this->Restaurant->find('list',
                                            array( 'fields' => array('Restaurant.id', 
                                                                     'Restaurant.name'),
                                            'recursive' => false,
        ...
        if ($this->request -> isPost()) {
            ...
            if ($this->User->RegisterValidate()) {
                ...
                $this->request->data['User']['salt'] = $salt;
                $this->request->data['User']['password'] = $this->UserAuth->makePassword($this->request->data['User']['password'], $salt);
                $this->request->data['User']['cpassword'] = $this->request->data['User']['password'];
//              ($this->request->data);
//              exit();
            ...
            if (!empty($user)) {
                if (!$user['User']['email_verified']) {
                    ...
                    if ($activateKey==$theKey) {
                        ...
                        $this->Session->setFlash(__('Thank you, your account has been activated'));
                    }
                }
                else {
                    $this->Session->setFlash(__('Your account is already activated'));
                }
            }
            else {
