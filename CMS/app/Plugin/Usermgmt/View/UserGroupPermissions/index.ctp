...
<script type="text/javascript">
      ...
      function chngtext(value){
          document.getElementById("updateDiv"+value).innerHTML="<font color='green'>Please saved changes</font>";
	...
	<div class="widget widget-blue">
        ...
        <h3><i class="icon-ok-sign"></i><?php echo __('User Group Permissions');?>
            <div style="float:right">   <span  class="umstyle2"><?php __('Select Controller');?></span>  <?php echo $this->Form->input("controller",array('type'=>'select','div'=>false,'options'=>$allControllers,'selected'=>$c,'label'=>false,"onchange"=>"window.location='".SITE_URL."permissions/?c='+(this).value"))?></h3>
</div>
							...
							for ($i=0; $i<count($value); $i++) {
								if (isset($value[$i])) {
									...
									foreach ($user_groups as $user_group) {
										...
										echo $this->Form->input($ugname,array('id'=>$ugname_alias.$k,'type'=>'checkbox','checked'=>$checked,'onChange'=>'chngtext('.$k.')'));
									}
									echo "</td>";
									echo "<td>";
