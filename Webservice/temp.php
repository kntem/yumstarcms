...
if(count($items)>0){
	...
	mysql_query("Insert into orders (user_id, status,total_amount) values (".$data[0].",1,0)") or die(mysql_error());
...
//echo "Insert into order_items (order_id, item_id, quantity,price) 
	//	select ".$lastOrderId.",".$data[2].",".$data[1].",i.price from items i where i.id=".$data[2]." limit 1";
		mysql_query("Insert into order_items (order_id, item_id, quantity,price) 
		select ".$lastOrderId.",".$data[2].",".$data[1].",i.price from items i where i.id=".$data[2]." limit 1
",$con) or die(mysql_error());

	}
	
	mysql_query("update orders set total_amount=(select ifnull(sum(price*quantity),0) from order_items where order_id=".$lastOrderId.")
				where id=".$lastOrderId) or die(mysql_error());
