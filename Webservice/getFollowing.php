...
$totalUserFollowingCount = mysql_query("select count(u.id) as 'totalFollowingCount' from `users_friends_mappings` um
								inner join users u on (u.id=um.friend_id)
								where user_id=" . $paramArray['userID'] . " and is_follow=true");

$followersList = mysql_query("select u.* from `users_friends_mappings` um
							inner join users u on (u.id=um.friend_id)
							where um.user_id=" . $paramArray['userID'] . " and is_follow=true order by u.id asc
...
if (mysql_num_rows($followersList) > 0) {
    ...
    $arr['status'] = "true";
    $arr['code'] = "P018";
    $arr['msg'] = "following list found";
} else {
    $arr['status'] = "false";
    $arr['code'] = "P019";
    $arr['msg'] = "following lsit not found";
}

$totalRows1 = "";
while ($row3 = mysql_fetch_array($totalUserFollowingCount)) {
    $totalRows1 = $row3['totalFollowingCount'];
}
