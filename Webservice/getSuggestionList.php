...
if ($flag == "1") {
    ...
    $desc = "";
    $fllowingsuggestion = "inner join (select friend_id from  users_friends_mappings where is_follow=1 and user_id=" . $userID . ")uz on uz.friend_id=u.id";
    $selectunion = "select * from (";
    $union = "union
					select sug.id as 'suggestionID',i.name as itemName,i.id as itemID, re.`name`as 'resturantName',
				sug.suggested_image,sug.suggested_comment,u.username,u.first_name,u.last_name,u.fb_id,u.id as userID,
					...
					second(Sec_to_time(Timestampdiff (second, '$curDate', sug.post_time))) as sec,
					minute(Sec_to_time(Timestampdiff (second, '$curDate', sug.post_time))) as min
					,HOUR(Sec_to_time(Timestampdiff (second, '$curDate', sug.post_time))) as hours
					,datediff(now(),sug.post_time) as days
					,floor(datediff(now(),sug.post_time) / 365) as years,sug.post_time from suggestions sug
...
$totalRows = mysql_query("select count(*) as 'totalCount' from (select sug.suggested_image,suggested_comment,u.fb_id,
						...
						order by post_time asc)ds") or die(mysql_error());
/*echo $selectunion . "select sug.id as 'suggestionID',i.name as itemName,i.id as itemID, re.`name`as 'resturantName',
				sug.suggested_image,sug.suggested_comment,u.username,u.first_name,u.last_name,u.fb_id,u.id as userID,
					...
					" . $userWhereCondition . "
					group by sug.id
					   " . $union . " " . $desc . " limit " . $paramArray['Limit1'] . ", " . $defaultRecordLimit;
					   exit;*/



$items = mysql_query($selectunion . "select sug.id as 'suggestionID',i.name as itemName,i.id as itemID, re.`name`as 'resturantName',
				sug.suggested_image,sug.suggested_comment,u.username,u.first_name,u.last_name,u.fb_id,u.id as userID,
					...
					" . $userWhereCondition . "
					group by sug.id
					   " . $union . " " . $desc . " limit " . $paramArray['Limit1'] . ", " . $defaultRecordLimit, $con) or die(mysql_error());
